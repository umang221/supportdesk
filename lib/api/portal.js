...
export async function updateOwnCustomerProfile(patch) {
  const response = await fetch("/api/portal/customers/me", {
    method: "PATCH",
    headers: { "Content-Type": "application/json" },
...
export async function changeOwnCustomerPassword({ currentPassword, newPassword }) {
  const response = await fetch("/api/portal/customers/me/password", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
export async function uploadOwnCustomerAvatar(file) {
  const formData = new FormData();
  formData.append("file", file);
  const response = await fetch("/api/portal/customers/me/avatar", { method: "POST", body: formData });
  return parseJsonResponse(response);
}
