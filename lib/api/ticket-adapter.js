 ...
 * was designed around the mock message fixtures. Only "User"-authored
 * messages are ever produced by the real API today (no customer session
 * exists to post as a Customer yet — see messageService.createMessage), so
 * authorType is always "agent" here.
 */
export function normalizeMessage(message) {
  ...
  return {
    ...
    authorType: "agent",
    authorName: message.author?.name ?? "Agent",
    body: message.body,
