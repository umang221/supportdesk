 ...
 * self-service change-password flow (customerAuthService.changeOwnPassword)
 * to exclude the session making the request from the "invalidate every
 * other session" cleanup, so changing your password doesn't also log you out.
 */
export async function getCurrentCustomerSessionToken() {
  const cookieStore = await cookies();
